...
const zip = (secondary, primary) => {
  const result = []
  let idx = 0
  while (idx < primary.length) {
    result[idx] = [secondary[idx], primary[idx]]
    idx += 1
  }
  return result
}

// Partially apply a function with arguments with a .equals function for
// comparison
export const partial = (fn) => (...args) => {
  const obj = (...more) => fn(...args, ...more)
  obj.__type = 'Partial'
  if (fn.__type === 'Partial') {
    obj.fn = fn.fn
    obj.args = fn.args.concat(args)
  } else {
    obj.fn = fn
    obj.args = args
  }
  obj.equals = (other) => partial.equals(other, obj)
  return obj
}

partial.equals = compareKeys(['__type', 'fn', 'args'])

// These functions define a .equals functions to specify how certain objects
// should be compared for tuning the lazy performance.
export const compare = {
  ...
  shallow: (obj) => {
    return R.map(value => {
      value.equals = (other) => value === other
      return value
    }, obj)
  },
}
